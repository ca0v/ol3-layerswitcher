...
export class LayerSwitcher extends ol.control.Control implements ILayerSwitcher {
  ...
  private afterCreate() {
    ...
    if (options.openOnClick || options.closeOnClick) {
      button.addEventListener("click", e => {
        this.isVisible()
          ? options.closeOnClick && this.hidePanel()
          : options.openOnClick && this.showPanel();
        e.preventDefault();
      });
    }
  }
  ...
  private ensureTopVisibleBaseLayerShown() {
    ...
    let visibleBaseLyrs = allLayers(this.getMap()).filter(
      l => l.get("type") === "base" && l.getVisible()
    );
    ...
    if ("getLayers" in lyr && !lyr.get("combine")) {
      if (!lyr.get("label-only")) {
        ...
        input.addEventListener("change", () => {
          ...
          this.state
            .filter(s => s.container === ul && s.input && s.input.checked)
            .forEach(state => {
